...
if [ -f "/etc/systemd/system/sshws.service" ]; then
	...
	wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/zakiii20211/Multiport/main/SSH/proxy3.js"
	cat <<EOF >/etc/systemd/system/sshws.service
[Unit]
Description=Websocket Enabler By khaiVPN
Documentation=https://zakiii20211-Project.net
[Service]
Type=simple
...
function start() {
	...
	if [[ ! -z "${PID}" ]]; then
		...
		wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/zakiii20211/Multiport/main/SSH/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler && /usr/bin/ssh-wsenabler
