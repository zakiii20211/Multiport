...
Name=$(curl https://raw.githubusercontent.com/zakiii20211/Multiport/refs/heads/main/LICENSE/access | grep $MYIP | awk '{print $2}')
...
restoreBot() {
	...
	echo -e "[ ${green}INFO${NC} ] • Restoring user data..."
	sleep 1
    cp /vmess/quota/* /etc/vmess/ &>/dev/null
    cp /vless/quota/* /etc/vless/ &>/dev/null
    cp /trojan/quota/* /etc/trojan/ &>/dev/null
    cp /vmess/ip/* /etc/kyt/limit/vmess/ip/ &>/dev/null
    cp /vless/ip/* /etc/kyt/limit/vless/ip/ &>/dev/null
    cp /trojan/ip/* /etc/kyt/limit/trojan/ip/ &>/dev/null
    cp /ssh/ip/* /etc/kyt/limit/ssh/ip/ &>/dev/null
    rm -rf /root/backup
    rm -f backup.zip
...
botbckpBot_menu() {
    ...
    echo -e " ${Blue}│$NC [${green}x${NC}] Back To Main Menu \e[0m"
    echo -e " ${Blue}╰═══════════════════════════════════════════════════════╯${NC}"
    echo -e ""
